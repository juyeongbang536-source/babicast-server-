...
app.post('/create-video', async (req, res) => {
  ...
  try {
    const scriptRes = await axios.post('https://api.anthropic.com/v1/messages', {
      ...
      max_tokens: 2000,
      system: '한국 SNS 숏폼 대본 전문가. JSON만 응답. {"title":"제목","keywords":["키워드1","키워드2"],"slides":[{"caption":"핵심한줄","text":"나레이션"}]}',
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      try {
        ...
        if (!videos || !videos.length) throw new Error('영상 없음');
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      const caption = slide.caption.replace(/'/g, "\\'").replace(/:/g, "\\:");
      const subtitleText = slide.text.replace(/'/g, "\\'").replace(/:/g, "\\:");
      await new Promise((res, rej) => {
        ffmpeg(videoFiles[i] || videoFiles[0])
          .seekInput(0).duration(5)
          .videoFilters([
            ...
            `drawtext=text='${subtitleText}':fontsize=38:fontcolor=white@0.85:x=60:y=h-200:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf:shadowcolor=black:shadowx=2:shadowy=2`,
          ])
